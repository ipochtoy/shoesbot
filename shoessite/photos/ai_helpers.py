...
def generate_product_summary(photo_data_list: List[Dict] = None, photo_urls: List[str] = None, barcodes: List[str] = None, gg_labels: List[str] = None) -> Optional[str]:
    ...
    try:
        ...
        content = [{
            'type': 'text',
            'text': f'''Ты эксперт по товарам и продажам. Проанализируй фото товара и создай стандартизированное описание товара для продажи на русском языке.

{context_text if context_text else ""}

Внимательно изучи все фото товара, упаковку, этикетки, бирки, баркоды и любые надписи.

Создай подробную сводку в следующем формате (ОБЯЗАТЕЛЬНО соблюдай структуру):

**Что это за товар:**
Краткое описание - что это за товар, для чего предназначен. 1-2 предложения, продающее описание.

**Бренд и модель:**
Бренд и точное название модели, если видно на упаковке.

**Размер и характеристики:**
Размер (например: M, L, 42, 10 и т.д.), цвет, материал, состав ткани если видно.

**Цвет:**
Укажи точный цвет товара, если видно на фото или упаковке (например: коричневый, черный, синий, бежевый и т.д.). Если цвет не видно - укажи "Не указан".

**Состояние товара:**
Определи состояние товара на основе фото:
- Если видна заводская упаковка, этикетки, бирки, товар в оригинальной упаковке или пакете - укажи "new" (новое)
- Если товар без упаковки, выглядит использованным - укажи "used" (б/у)
- Если не можешь определить - укажи "new" по умолчанию

**Особенности:**
Особые свойства, технологии, защита (UPF, антибактериальное и т.д.), если указано. Перечисли через запятую или точку с запятой.

**Температурный режим / Назначение:**
Если указано на упаковке - для каких температур или условий предназначен.

**Производитель:**
Страна производства, если видно.

**Рекомендуемая розничная цена (USD):**
{price_instruction}

**Описание для продажи:**
Создай продающее описание товара (3-5 предложений) для использования в интернет-магазине. Опиши преимущества, для кого подходит, в каких ситуациях использовать. Пиши привлекательно, но правдиво, используя информацию с упаковки и бирок.

Сделай сводку подробной, информативной, но читаемой. Используй информацию напрямую с упаковки и бирок. Если чего-то не видно - не придумывай.'''
        }]
        ...
        payload = {
            ...
            'max_tokens': 1200,  # Уменьшено с 1500 для ускорения
            'temperature': 0.3,
        }
